...
VOWEL = 'АУОЫИЭЯЮЁЕауоыиэяюёе'

if __name__ == '__main__':   # Код в этом блоке запускается только при прямом вызове скрипта (python task34.py), и не запускается при импорте
    phrases = input().split()

    counts = list(map(
        lambda phrase: len(list(filter(
            lambda char: char in VOWEL,
             phrase
            ))),
         phrases
         ))
